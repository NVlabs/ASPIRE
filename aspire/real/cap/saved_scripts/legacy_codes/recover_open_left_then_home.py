...
"""Recovery-only: open the left gripper in place, then go home.

Use after a failed left-arm grasp attempt where the gripper may be near an
...
RESULT: dict[str, Any] = {
    "success": False,
    "method": "recover_open_left_then_home",
...
def _run_dir() -> Path:
    try:
        ...
        return current_run_dir(default=f"logs/recover_open_left_then_home_{time.strftime('%Y%m%dT%H%M%S')}")
    except Exception:
        return Path(f"logs/recover_open_left_then_home_{time.strftime('%Y%m%dT%H%M%S')}")
...
try:
    ...
    RESULT["open_left"] = _json_safe(set_gripper("left", 1.0, vel_limit=0.18, torque_limit=0.12))
    ...
    RESULT["success"] = True
    RESULT["why_stopped"] = "opened left gripper in place, then go_home completed"

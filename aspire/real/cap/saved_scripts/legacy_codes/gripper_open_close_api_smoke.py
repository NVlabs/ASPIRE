"""Physical open_gripper/close_gripper API smoke test for YAM.

This script commands one gripper through open/close API calls and records
encoder feedback after each step. It does not command arm motion. Physical
gripper motion is refused unless OPENFORGE_ALLOW_PHYSICAL_MOTION=1.
"""
...
TASK_NAME = "gripper_open_close_api_smoke"
...
def _csv_actions(name: str, default: tuple[str, ...]) -> list[str]:
    raw = os.environ.get(name)
    if raw is None or not raw.strip():
...
def get_task_info() -> dict[str, Any]:
    return dict(globals().get("result") or {"method": TASK_NAME})
...
side = os.environ.get("OPENFORGE_GRIPPER_OC_SMOKE_SIDE", "left").strip().lower()
actions = _csv_actions("OPENFORGE_GRIPPER_OC_SMOKE_ACTIONS", ("open", "close", "open"))
settle_s = _env_float("OPENFORGE_GRIPPER_OC_SMOKE_SETTLE_S", 1.0)
vel_limit = _env_float_optional("OPENFORGE_GRIPPER_OC_SMOKE_VEL_LIMIT")
torque_limit = _env_float_optional("OPENFORGE_GRIPPER_OC_SMOKE_TORQUE_LIMIT")
tolerance = _env_float("OPENFORGE_GRIPPER_OC_SMOKE_TOLERANCE", 0.08)

result: dict[str, Any] = {
    "schema": "openforge.gripper_open_close_api_smoke.v1",
    "success": False,
    "method": TASK_NAME,
    "run_dir": str(run_dir),
...
try:
    ...
    for action in actions:
        if action not in {"open", "close"}:
            raise RuntimeError(f"invalid action {action!r}; expected open or close")
        fn = open_gripper if action == "open" else close_gripper
        target = 1.0 if action == "open" else 0.0
        before = _read_gripper(get_robot_state, side)
        command_result = _call_gripper(fn, side, vel_limit, torque_limit)
        result["physical_motion_executed"] = True
        result["movement_capable_calls"].append(f"{action}_gripper:{side}")
        time.sleep(settle_s)
        after = _read_gripper(get_robot_state, side)
        reached = after is not None and abs(float(after) - target) <= tolerance
        step = {
            "action": action,
            "target": target,
            "before": before,
            ...
            "reached": reached,
            "error_abs": None if after is None else abs(float(after) - target),
            "command_result": _json_safe(command_result),
        }
        result["steps"].append(step)
        print(f"[{TASK_NAME}] {side} action={action} before={before} after={after} reached={reached}")
    ...
    result["why_stopped"] = "open/close gripper API smoke completed"
except Exception as exc:
    result["success"] = False
    result["why_stopped"] = f"open/close gripper API smoke failed/refused: {type(exc).__name__}: {exc}"
finally:

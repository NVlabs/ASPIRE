"""After-analysis guarded white-bowl pickup/lift script for real YAM.
...
It encodes the 2026-05-30 bowl-analysis lessons for the shallow white bowl:
explicit white-bowl verification, compensated TCP below estimated rim, and
current top-camera table-Z metadata.
"""
...
def _configure_white_bowl_defaults() -> None:
    _set_default_env("OPENFORGE_BOWL_RACK_TASK_NAME", "place_white_bowl_on_rack_one_shot_loop_after_analysis")
    _set_default_env("OPENFORGE_BOWL_RACK_OBJECT_NAME", "white bowl")
    _set_default_env("OPENFORGE_BOWL_RACK_ARTIFACT_PREFIX", "white_bowl_after_analysis")
    _set_default_env(
        "OPENFORGE_BOWL_RACK_SCRIPT_PATH",
        "cap/saved_scripts/place_white_bowl_on_rack_one_shot_loop_after_analysis.py",
    )
    _set_default_env(
    ...
    _set_default_env("OPENFORGE_BOWL_RACK_REQUIRE_EXPLICIT_PICKUP_PROMPT", "1")
    _set_default_env("OPENFORGE_BOWL_RACK_PICKUP_EXPLICIT_PROMPTS", "white bowl")
    ...
    _set_default_env("OPENFORGE_BOWL_RACK_PRE_CLOSE_MAX_CONTACT_ABOVE_OBJECT_Z_M", "0.070")
    _set_default_env("OPENFORGE_BOWL_RACK_REQUIRE_CONTACT_BEFORE_LIFT", "0")
    _set_default_env("OPENFORGE_BOWL_RACK_VERIFY_REQUIRE_EXPLICIT_TARGET_PROMPT", "1")
    _set_default_env("OPENFORGE_BOWL_RACK_VERIFY_TARGET_PROMPTS", "white bowl")
    _set_default_env("OPENFORGE_BOWL_RACK_ANALYSIS_TABLE_Z_M", "0.760")
    _set_default_env("OPENFORGE_BOWL_RACK_ANALYSIS_OPERATOR_HEIGHT_M", "0.005")
    _set_default_env("OPENFORGE_BOWL_RACK_ANALYSIS_OPERATOR_RADIUS_M", "0.095")
    _set_default_env("OPENFORGE_PLATE_USE_TOP_BOX_RADIUS", "1")
    _set_default_env("OPENFORGE_PLATE_DEFAULT_RADIUS_M", "0.095")
    ...
    _set_default_env("OPENFORGE_BOWL_TOP_DOWN_TCP_ABOVE_RIM_M", "-0.040")
    _set_default_env("OPENFORGE_BOWL_TOP_DOWN_LIFT_Z_M", "0.100")
...
TASK_NAME = os.environ.get("OPENFORGE_BOWL_RACK_TASK_NAME", "place_white_bowl_on_rack_one_shot_loop_after_analysis").strip()
TASK_NAME = TASK_NAME or "place_white_bowl_on_rack_one_shot_loop_after_analysis"
...
TASK_SCRIPT_PATH = os.environ.get(
    "OPENFORGE_BOWL_RACK_SCRIPT_PATH",
    "cap/saved_scripts/place_white_bowl_on_rack_one_shot_loop_after_analysis.py",
).strip() or "cap/saved_scripts/place_white_bowl_on_rack_one_shot_loop_after_analysis.py"
...
def _analysis_context(selected_pickup: dict[str, Any] | None = None) -> dict[str, Any]:
    table_z = _env_float("OPENFORGE_BOWL_RACK_ANALYSIS_TABLE_Z_M", 0.760)
    operator_height = _env_float("OPENFORGE_BOWL_RACK_ANALYSIS_OPERATOR_HEIGHT_M", 0.005)
    operator_radius = _env_float("OPENFORGE_BOWL_RACK_ANALYSIS_OPERATOR_RADIUS_M", 0.095)
    context: dict[str, Any] = {
        "schema": "openforge.white_bowl.after_analysis_context.v1",
        "source_note": "historical operator analysis",
        "table_z_reference_m": table_z,
        "table_z_source": "2026-05-30T204215 debug_observation_smoke top-camera table prompt cluster",
        "operator_white_bowl_height_m": operator_height,
        "operator_white_bowl_radius_m": operator_radius,
        "policy": (
            "White bowl pickup uses fresh live detections, explicit white-bowl verification, "
            "and a compensated TCP below estimated rim because depth/rim Z is known to be high."
        ),
        "bottom_camera_policy": "debug/image-only; never used for motion, ranking, close width, or success verification",
        "known_gaps": [
            "white bowl rim_z is a world-frame estimate and likely overestimates true shallow bowl height",
            "top-box-scaled radius is useful evidence from the successful run but not physical ground truth",
            "hard encoder contact may remain false even when target_reached and plausible_for_lift are usable",
        ],
        "artifact_correction": (
            "bowl_analysis.md lists white close target as 0.03, but the 20260530T195539 "
            "executed staged-close artifact shows target 0.04 and final position about 0.0481."
        ),
    }
    if selected_pickup:
        rim = selected_pickup.get("rim_geometry_estimate") or {}
        rim_z = rim.get("estimated_rim_z_m")
        if rim_z is not None:
            try:
                context["estimated_rim_height_above_table_m"] = round(float(rim_z) - table_z, 5)
                context["estimated_rim_height_minus_operator_height_m"] = round(float(rim_z) - table_z - operator_height, 5)
            except (TypeError, ValueError):
                pass
        radius_estimate = rim.get("radius_estimate") or selected_pickup.get("radius_estimate") or {}
        if radius_estimate:
            context["radius_estimate"] = radius_estimate
            try:
                context["radius_minus_operator_radius_m"] = round(float(radius_estimate.get("radius_m")) - operator_radius, 5)
            except (TypeError, ValueError):
                pass
        context["tcp_above_rim_m"] = rim.get("tcp_above_rim_m")
        context["lift_z_m"] = _env_float("OPENFORGE_BOWL_TOP_DOWN_LIFT_Z_M", 0.100)
        context["rim_angle_deg"] = selected_pickup.get("rim_angle_deg")
        context["verification_prompt_policy"] = {
            "require_explicit_target_prompt": _truthy_env(
                "OPENFORGE_BOWL_RACK_VERIFY_REQUIRE_EXPLICIT_TARGET_PROMPT",
                True,
            ),
            "target_prompts": _csv("OPENFORGE_BOWL_RACK_VERIFY_TARGET_PROMPTS", [TASK_OBJECT_NAME]),
        }
    return context
...
def _validate_pickup_strategy(planner_result: dict[str, Any]) -> dict[str, Any]:
    ...
    pickup_prompt = str(source.get("prompt") or "").strip().lower()
    explicit_prompts = {prompt.strip().lower() for prompt in _csv("OPENFORGE_BOWL_RACK_PICKUP_EXPLICIT_PROMPTS", [TASK_OBJECT_NAME])}
    require_explicit_pickup = _truthy_env("OPENFORGE_BOWL_RACK_REQUIRE_EXPLICIT_PICKUP_PROMPT", True)
    explicit_pickup_ok = (not require_explicit_pickup) or pickup_prompt in explicit_prompts
    ...
    return {
        ...
        "pickup_prompt": pickup_prompt or None,
        "require_explicit_pickup_prompt": require_explicit_pickup,
        "explicit_pickup_prompts": sorted(explicit_prompts),
        "explicit_pickup_ok": explicit_pickup_ok,
...
def _select_object_detection(
    ...
    detections = [dict(det) for det in packet.get("all_detections") or []]
    prompt_rank = {str(prompt).strip().lower(): index for index, prompt in enumerate(preferred_prompts)}
    filtered = []
    for det in detections:
        prompt = str(det.get("prompt") or "").lower()
        camera = str(det.get("source_camera") or det.get("camera") or "")
...
def _lift_verification_prompts() -> tuple[str, ...]:
    prompts = _csv("OPENFORGE_BOWL_RACK_VERIFY_TARGET_PROMPTS", [TASK_OBJECT_NAME])
    normalized = tuple(str(prompt).strip().lower() for prompt in prompts if str(prompt).strip())
    return normalized or (TASK_OBJECT_NAME.lower(),)


def _select_lift_verification_detection(packet: dict[str, Any]) -> tuple[dict[str, Any] | None, dict[str, Any]]:
    require_explicit = _truthy_env("OPENFORGE_BOWL_RACK_VERIFY_REQUIRE_EXPLICIT_TARGET_PROMPT", True)
    prompts = _lift_verification_prompts()
    if require_explicit:
        selected = _select_object_detection(packet, preferred_prompts=prompts)
        policy = {
            "require_explicit_target_prompt": True,
            "target_prompts": list(prompts),
            "selected_prompt": (selected or {}).get("prompt"),
            "selected_label": (selected or {}).get("label"),
            "reason": "explicit target prompt required so generic bowl cannot verify white-bowl lift",
        }
        return selected, policy
    selected = _select_object_detection(packet)
    return selected, {
        "require_explicit_target_prompt": False,
        "target_prompts": list(prompts),
        "selected_prompt": (selected or {}).get("prompt"),
        "selected_label": (selected or {}).get("label"),
        "reason": "generic fallback allowed by env override",
    }
...
def _execute_pickup_lift_only(run_dir: Path, selected_pickup: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        open_step = _set_gripper_checked(
            ...
            vel_limit=_env_float("OPENFORGE_BOWL_RACK_OPEN_VEL_LIMIT", 0.35),
            torque_limit=_env_float("OPENFORGE_BOWL_RACK_OPEN_TORQUE_LIMIT", 0.18),
        )
        ...
        def _open_and_retreat(stage_prefix: str) -> None:
            release = _set_gripper_checked(
                ...
                vel_limit=_env_float("OPENFORGE_BOWL_RACK_RELEASE_VEL_LIMIT", 0.25),
                torque_limit=_env_float("OPENFORGE_BOWL_RACK_RELEASE_TORQUE_LIMIT", 0.12),
            )
            attempt["movement_capable_calls"].append(f"set_gripper:{side}")
        ...
        close = staged_close_with_contact(
            ...
            vel_limit=_env_float("OPENFORGE_BOWL_RACK_CLOSE_VEL_LIMIT", float(close_plan.get("vel_limit", 0.05))),
            torque_limit=_env_float("OPENFORGE_BOWL_RACK_CLOSE_TORQUE_LIMIT", float(close_plan.get("torque_limit", 0.12))),
        ...
        post_lift_det, verification_detection_policy = _select_lift_verification_detection(post_lift)
        attempt["post_lift_observation"] = post_lift
        attempt["lift_verification_detection_policy"] = verification_detection_policy
...
def _run() -> dict[str, Any]:
    ...
    try:
        ...
        result["analysis_context"] = _analysis_context(selected_pickup)
        write_json(run_dir / "plans" / f"{TASK_ARTIFACT_PREFIX}_analysis_context.json", result["analysis_context"])
        append_stage_summary(
            run_dir,
            [
                "## white bowl after-analysis context",
                f"- table_z_reference_m: {result['analysis_context'].get('table_z_reference_m')}",
                f"- estimated_rim_height_above_table_m: {result['analysis_context'].get('estimated_rim_height_above_table_m')}",
                f"- tcp_above_rim_m: {result['analysis_context'].get('tcp_above_rim_m')}",
                f"- lift_z_m: {result['analysis_context'].get('lift_z_m')}",
                f"- verification_prompt_policy: {result['analysis_context'].get('verification_prompt_policy')}",
            ],
        )

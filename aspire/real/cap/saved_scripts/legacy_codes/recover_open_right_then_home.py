...
RESULT: dict[str, Any] = {
    "success": False,
    "method": "recover_open_right_then_home",
...
def _run_dir() -> Path:
    try:
        ...
        return current_run_dir(default=f"logs/recover_open_right_then_home_{time.strftime('%Y%m%dT%H%M%S')}")
    except Exception:
        return Path(f"logs/recover_open_right_then_home_{time.strftime('%Y%m%dT%H%M%S')}")
...
try:
    ...
    RESULT["open_right"] = _json_safe(set_gripper("right", 1.0, vel_limit=0.18, torque_limit=0.12))
    RESULT["physical_motion_executed"] = True
    RESULT["movement_capable_calls"].append("set_gripper:right")
    time.sleep(0.4)
    ...
    RESULT["success"] = True
    RESULT["why_stopped"] = "opened right gripper in place, then go_home completed"

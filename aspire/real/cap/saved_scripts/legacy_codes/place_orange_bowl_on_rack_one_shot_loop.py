...
"""Guarded orange-bowl-to-rack one-shot script for real YAM.
...
TASK_NAME = "place_orange_bowl_on_rack_one_shot_loop"
...
def _source_snapshot(run_dir: Path) -> dict[str, Any]:
    script_path = Path("cap/saved_scripts/place_orange_bowl_on_rack_one_shot_loop.py").resolve()
    snapshot = {
...
def _run_fresh_pickup_planner() -> dict[str, Any]:
    ...
    overrides = {
        ...
        "OPENFORGE_PLATE_PROMPTS": os.environ.get("OPENFORGE_BOWL_RACK_PICKUP_PROMPTS", "orange bowl,bowl,white bowl"),
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("openforge_orange_bowl_pickup_plan", planner_path)
        ...
        namespace = runpy.run_path(spec.origin, init_globals=tool_globals, run_name="__openforge_orange_bowl_plan__")
...
def _preview_place_sequence(
    ...
    for candidate in place_candidates:
        ...
        if success:
            ...
            packet["path"] = write_json(run_dir / "plans" / "orange_bowl_to_rack_sequence_preview.json", packet)
            append_stage_summary(
                run_dir,
                [
                    "## orange bowl to rack sequence preview",
                    "- success: True",
...
def _preview_pickup_lift_sequence(run_dir: Path, selected_pickup: dict[str, Any]) -> dict[str, Any]:
    ...
    packet["path"] = write_json(run_dir / "plans" / "orange_bowl_pickup_lift_sequence_preview.json", packet)
    append_stage_summary(
        run_dir,
        [
            "## orange bowl pickup/lift sequence preview",
            f"- success: {success}",
...
def _ticket_status(run_dir: Path) -> dict[str, Any]:
    ...
    if scope != "orange_bowl_to_rack":
        missing.append("OPENFORGE_BOWL_RACK_TICKET_SCOPE=orange_bowl_to_rack")
...
def _set_gripper_checked(
    *,
    ...
    if not ok:
        error = (
            f"gripper state {after} outside confirmed window "
            f"[{confirm_min if confirm_min is not None else '-inf'}, "
            f"{confirm_max if confirm_max is not None else 'inf'}]"
        )
    return {
        "stage": stage,
        "side": side,
...
def _capture_object_observation(run_dir: Path, stage: str) -> dict[str, Any]:
    prompts = _csv("OPENFORGE_BOWL_RACK_POST_PROMPTS", ["orange bowl", "bowl", "rack", "wooden rack", "blue gripper fingers"])
    packet = capture_scene(
        prompts=prompts,
        cameras=_debug_observation_cameras(),
        detect_objects_oneshot=_required_tool("detect_objects_oneshot"),
...
def _select_object_detection(packet: dict[str, Any], preferred_prompts: tuple[str, ...] = ("orange bowl", "bowl")) -> dict[str, Any] | None:
    detections = [dict(det) for det in packet.get("all_detections") or []]
...
def _pre_close_visual_gate(run_dir: Path, selected_pickup: dict[str, Any], observation: dict[str, Any]) -> dict[str, Any]:
    ...
    packet["path"] = write_json(run_dir / "plans" / "orange_bowl_pre_close_visual_gate.json", packet)
    append_stage_summary(
        run_dir,
        [
    ...
    *,
    ...
    if post is None or not release_target:
        return {
            "success": False,
            "confidence": 0.2,
            "reason": "missing post-place orange bowl detection or release target",
            "post_detection": post,
    ...
    return {
        ...
        "reason": "orange bowl detected near rack placement target" if success else "post detection not close enough to rack target",
        "post_detection": post,
        "object_motion": motion,
...
def _execute_pickup_lift_only(run_dir: Path, selected_pickup: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        pregrasp_open = _preshape_gripper(selected_pickup)
        preshape_max = pregrasp_open + _env_float("OPENFORGE_BOWL_RACK_PRESHAPE_TOLERANCE_POS", 0.08)
        hold = _hold_gripper(selected_pickup)
...
def _execute_physical(run_dir: Path, selected_pickup: dict[str, Any], selected_place: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        attempt["success"] = bool(attempt["verification_success"])
        attempt["why_stopped"] = (
            "orange bowl placement verified on/near rack"
            if attempt["success"]
...
def _run() -> dict[str, Any]:
    ...
    try:
        ...
        if strategy_env_problems:
            result["why_stopped"] = "refusing non-top-down orange-bowl pickup strategy configuration"
        ...
        if _truthy_env("OPENFORGE_BOWL_RACK_PICKUP_LIFT_ONLY", False):
            ...
            if not pickup_lift_preview.get("success"):
                ...
                result["next_recommendation"] = "Inspect orange_bowl_pickup_lift_sequence_preview.json before physical motion."
                return result
            if not result["physical_gate"]:
                ...
                result["next_recommendation"] = (
                    "For physical pickup/lift validation, rerun under the active orange_bowl_to_rack ticket "
        ...
        result["place_candidates_path"] = write_json(
            run_dir / "plans" / "orange_bowl_rack_place_candidates.json",
            {
        ...
        if not result["physical_gate"]:
            ...
            result["next_recommendation"] = (
                "For physical mode, rerun once under the active orange_bowl_to_rack ticket "
                "with OPENFORGE_ALLOW_PHYSICAL_MOTION=1 and complete precondition env vars."
            )
            return result

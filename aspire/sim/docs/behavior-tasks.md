...
| Component | Value |
|---|---|
| Host | OSMO node, 4× NVIDIA L40 (48 GB each), 128 cores |
| Driver | 580.95.05 |
...
It finishes by running `scripts/verify_behavior.py`, which fails loudly rather
...
> `scripts/setup_behavior.sh` invokes it for the pinned Isaac Sim wheel set and
...
python scripts/verify_behavior.py --gpu-id 2   # full: servers + oracle seed
python scripts/verify_behavior.py --quick      # imports, pins, datasets only
```
...
`scripts/setup_behavior.sh` installs `aspire` as an editable package into the
...
`scripts/verify_behavior.py` retries the oracle seed once (`--attempts`) for this
reason.
...
|---|---|
| `ModuleNotFoundError: No module named 'cv2'` during install | Nothing in the dependency chain installs OpenCV, and the upstream installer's verification step imports it. `scripts/setup_behavior.sh` pre-installs `opencv-python-headless`. |

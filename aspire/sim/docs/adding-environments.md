...
Subclass `BaseEnv` in `cap/envs/simulators/`. Implement the five required methods:

```python
# cap/envs/simulators/my_sim.py
...
Register in `cap/envs/simulators/__init__.py`:

```python
from aspire.sim.cap.envs.base import register_env
...
```python
# cap/envs/tasks/my_robot/my_task.py
...
Register in `cap/envs/tasks/__init__.py`:

```python
from aspire.sim.cap.envs.tasks.base import CodeExecEnvConfig, register_exec_env, register_config
...
3. Run:
```bash
uv run --no-sync --active python -m aspire.sim.cap.envs.launch --config-path env_configs/r1pro/${TASK_NAME}.yaml
```

## Listing available environments

```python
from aspire.sim.cap.envs.tasks import list_exec_envs, list_configs
from aspire.sim.cap.envs import list_envs
from aspire.sim.cap.integrations import list_apis

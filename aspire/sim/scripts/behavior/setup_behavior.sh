...
usage() {
  cat <<EOF
Usage: scripts/behavior/setup_behavior.sh [OPTIONS]
...
if [[ "${SKIP_VERIFY}" == true ]]; then
  ...
  echo "Environment NOT verified. Run: scripts/behavior/verify_behavior.py"
  exit 0
fi

log "Verifying the environment"
OMNIGIBSON_GPU_ID="${GPU_ID}" "${PY}" "${SIM_ROOT}/scripts/behavior/verify_behavior.py" --gpu-id "${GPU_ID}"

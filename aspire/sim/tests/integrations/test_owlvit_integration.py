...
def test_owlvit_open_vocab_detection_real() -> None:
    if os.environ.get("ASPIRE_INTEGRATION_REAL", "0") != "1":
        pytest.skip("Set ASPIRE_INTEGRATION_REAL=1 to run real OWL-ViT test")

    detect_open_vocab = init_owlvit(device=os.environ.get("ASPIRE_DEVICE", "cpu"))

    url = os.environ.get(
        "ASPIRE_TEST_IMAGE_URL", "http://images.cocodataset.org/val2017/000000039769.jpg"
    )

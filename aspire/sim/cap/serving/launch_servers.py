"""Unified server launcher for ASPIRE.

Start all API servers needed for an environment config with one command:

    uv run --no-sync --active cap/serving/launch_servers.py \
...
Or start a predefined server profile:
    ...
    uv run --no-sync --active cap/serving/launch_servers.py --profile full
"""
...
SERVER_REGISTRY: dict[str, dict[str, Any]] = {
    "sam3": {
        "target": "aspire.sim.cap.serving.launch_sam3_server",
        "default_port": 8114,
        "gpu_required": True,
        "gpu_memory_mb": 3000,
    ...
    "graspnet": {
        "target": "aspire.sim.cap.serving.launch_contact_graspnet_server",
        "default_port": 8115,
        "gpu_required": True,
        "gpu_memory_mb": 2000,
        "extra_args": {"device": "cuda"},
    ...
    "pyroki": {
        "target": "aspire.sim.cap.serving.launch_pyroki_server",
        "default_port": 8116,
        "gpu_required": False,
        "gpu_memory_mb": 0,
        "extra_args": {},
    ...
    "sam2": {
        "target": "aspire.sim.cap.serving.launch_sam2_server",
        "default_port": 8113,
        "gpu_required": True,
        "gpu_memory_mb": 6000,
        "extra_args": {"device": "cuda"},
    ...
    "owlvit": {
        "target": "aspire.sim.cap.serving.launch_owlvit_server",
        "default_port": 8118,
        "gpu_required": True,
        "gpu_memory_mb": 3000,
        "extra_args": {"device": "cuda"},
    ...
    "curobo": {
        "target": "aspire.sim.cap.serving.launch_curobo_server",
        "default_port": 8117,
        "gpu_required": True,
        "gpu_memory_mb": 2000,
        "extra_args": {},
...
def parse_servers_from_yaml(config_path: str) -> list[dict[str, Any]]:
    """Extract server definitions from a ASPIRE YAML config.

...
Extracted from cap/utils/launch_utils.py to separate LLM query logic
...
VLM_MODELS = [
    ...
    "gcp/google/gemini-3.1-pro-preview",  # NVIDIA inference API prefix
    "google/gemini-2.5-flash-lite",
    "anthropic/claude-opus-4-5",
    "anthropic/claude-haiku-4-5",
    "aws/anthropic/bedrock-claude-opus-4-6",  # NVIDIA inference API (AWS Bedrock)
    "openai/gpt-5.4",
    "openai/o1",
    "openai/o4-mini",
...
ENSEMBLE_CONFIGS = [
    ...
    ("aws/anthropic/bedrock-claude-opus-4-6", [0.1, 0.5, 0.9]),
]
...
def query_model(args: "LaunchArgs | ModelQueryArgs", prompt: list[dict]) -> str:
    ...
    response = requests.post(
        server_url, headers=headers, data=json.dumps(payload), timeout=500
    )
    retry = 1
    while response.status_code in [404, 429, 500, 502, 503, 504]:
        sleep_time = 30 + random.uniform(-10, 10) if response.status_code == 429 else 120 + random.uniform(-40, 40)
        print(f"Retry {retry}. Model query failed with status code {response.status_code}. Error: {response.text}. Retrying in {sleep_time:.1f} seconds...")
...
def query_model_ensemble(
    args: "LaunchArgs | ModelQueryArgs",
    prompt: list[dict],
    synthesis_model: str = "aws/anthropic/bedrock-claude-opus-4-6",
    is_multiturn = False
) -> dict[str, Any]:

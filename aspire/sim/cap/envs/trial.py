"""Single-trial execution for ASPIRE environments.
...
from aspire.sim.cap.utils.video_utils import _encode_video_base64, _write_video
...
def _handle_multi_turn_step(
    ...
    wrist_turn_frames: list[np.ndarray] | None = None,
    wrist_base64_history: list[str] | None = None,
) -> tuple[str, str | None, str | None, dict | None, list | None, str | None]:
    ...
    return decision, new_code, reasoning, multiturn_ensemble_entry, decision_prompt, differencing_feedback
...
def _run_single_trial(
    ...
    differencing_feedback_history: list[str] = []
    ...
    while code_block_idx < len(code_blocks) and code_block_idx <= MULTITURN_LIMIT:
        ...
        if multi_turn_prompt:
            ...
            decision, new_code, mt_reasoning, mt_ensemble, decision_prompt, mt_diff_feedback = _handle_multi_turn_step(
            ...
            if mt_diff_feedback is not None:
                differencing_feedback_history.append(mt_diff_feedback)
    ...
    if code_path and hasattr(env, "_apis"):
        from pathlib import Path as _Path
        _trial_dir = _Path(code_path).parent
        for _api in env._apis.values():
            if hasattr(_api, "get_trace_logger"):
                _api.get_trace_logger().save(_trial_dir)
                _api.get_trace_logger().reset()
    ...
    if config.get("evolve_skill_library", False) and info_step.get("task_completed", False):
        try:
            from aspire.sim.cap.skills import SkillLibrary

...
np.save(f"debug_grasping/object_pc_points_world.npy", mask_pc_dict["points_3d"])
filtered_points, _ = filter_noise(mask_pc_dict["points_3d"])
np.save(f"debug_grasping/object_pc_points_world_filtered.npy", filtered_points)
...
Image.fromarray(ext_rgb).save(f"debug_grasping/external_cam.png")
Image.fromarray(wrist_rgb).save(f"debug_grasping/wrist_cam.png")
_overlay_mask_red(ext_rgb, mask_pc_dict["agentview_mask"]).save(f"debug_grasping/external_cam_obj_mask.png")
_overlay_mask_red(wrist_rgb, mask_pc_dict["wrist_mask"]).save(f"debug_grasping/wrist_cam_obj_mask.png")
_overlay_mask_red(wrist_rgb, wrist_sam3_mask["mask"]).save(f"debug_grasping/wrist_cam_sam3_mask.png")
_overlay_mask_red(ext_rgb, agentview_sam3_mask["mask"]).save(f"debug_grasping/external_cam_sam3_mask.png")
"""
...
while True:
    if current_state == 'find_object':
        ...
        # wrist_rgb = obs["robot0_eye_in_hand"]["images"]["rgb"]
        # import PIL.Image as Image
        # Image.fromarray(wrist_rgb).save(f"debug_grasping/wrist_rgb.png")
        if response:
            break
        else:
            current_state = 'find_object'

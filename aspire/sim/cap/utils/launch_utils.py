...
def _load_config(args: LaunchArgs) -> tuple[Any, dict[str, Any], list]:
    ...
    if (
        getattr(args, "use_legacy_multi_turn_decision_prompt", None) is None
        and "use_legacy_multi_turn_decision_prompt" in configs_dict
    ):
        setattr(
            args,
            "use_legacy_multi_turn_decision_prompt",
            configs_dict["use_legacy_multi_turn_decision_prompt"],
        )
...
def _save_trial_artifacts(
    ...
    trial_dir_name = f"trial_{trial:02d}_sandboxrc_{sandbox_rc}_reward_{reward:.3f}_taskcompleted_{int(task_completed)}"
    trial_dir = output_path / trial_dir_name

    # Reuse existing folder for this trial (reward and sandbox_rc may change
    # between multi-turn code blocks).  Rename it to reflect the latest state.
    if not trial_dir.exists():
        prefix = f"trial_{trial:02d}_"
        for existing in output_path.glob(f"{prefix}*"):
            if existing.is_dir():
                existing.rename(trial_dir)
                break
    ...
    if differencing_feedback_history:
        for i, text in enumerate(differencing_feedback_history):
            (trial_dir / f"differencing_feedback_{i:02d}.txt").write_text(text)

...
from aspire.sim.cap.integrations.franka.common import (
    apply_tcp_offset,
...
from aspire.sim.cap.integrations.vision.graspnet import init_contact_graspnet, init_contact_graspnet_point_clouds
from aspire.sim.cap.integrations.vision.molmo import init_molmo
from aspire.sim.cap.integrations.vision.sam3 import init_sam3, init_sam3_point_prompt
...
from aspire.sim.cap.utils.depth_utils import (
...
def _get_curobo_api():
    ...
    if _curobo_api is None:
        from aspire.sim.cap.integrations.motion import curobo_api as _mod
        _curobo_api = _mod
    return _curobo_api
...
class FrankaLiberoApiReduced(ApiBase):
    ...
    def __init__(self, env: BaseEnv) -> None:
        ...
        # Check if Molmo server is reachable before initializing
        import requests as _requests
        try:
            _requests.get("http://127.0.0.1:8122/v1/models", timeout=2)
            self._molmo_available = True
        except Exception:
            self._molmo_available = False
            print("[API] Molmo server not reachable — point_prompt_molmo will be excluded from API.")
        if self._molmo_available:

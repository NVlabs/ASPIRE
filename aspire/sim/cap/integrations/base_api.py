...
class ApiBase(ABC):
    ...
    def _log_step(
        ...
        This is a thin wrapper around :func:`aspire.sim.cap.utils.execution_logger.log_step`
        that no-ops when the web UI flag is off, keeping control API code free of
        conditional checks.
        """
        if not self._webui_enabled:
            return
        from aspire.sim.cap.utils.execution_logger import log_step
    ...
    def _log_step_update(
        ...
        if not self._webui_enabled:
            return
        from aspire.sim.cap.utils.execution_logger import log_step_update

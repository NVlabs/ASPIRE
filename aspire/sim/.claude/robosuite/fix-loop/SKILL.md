...
| **Robosuite Fix Loop** | [main-agent-prompt.md](main-agent-prompt.md) | Coordinator guide: dispatch one subagent per GPU to debug failures |
| **Robosuite Fix Loop Subagent** | [subagent-prompt.md](subagent-prompt.md) | Self-contained prompt template for dispatching one task to a background subagent |
...
6. **Update the experiment's `skills/`** (`.claude/robosuite/fix-loop/skills/`) after any new pattern discovered

...
1. Read `.claude/robosuite/training-law/subagent-prompt.md`
2. Copy the template block (everything inside the ``` fences) **VERBATIM** — the ENTIRE block, for EVERY task
...
YOU MUST NOT add extra context, reorder sections, summarize sections, or omit any part of the template. The template contains precise formatting requirements (e.g. reasoning.txt format) that will be silently lost if you rewrite anything. 

**⚠️ COMMON MISTAKE: Do NOT abbreviate or summarize the template for later tasks to save tokens.** Every subagent gets the FULL template. Abbreviated prompts cause subagents to improvise their own conventions (wrong file naming, skipping result.json, etc.).
...
Agent(
    ...
    model="opus",
    prompt=<filled-in template from .claude/robosuite/training-law/subagent-prompt.md>,
...
1. Read the subagent prompt template from `.claude/robosuite/training-law/subagent-prompt.md`
2. Fill in TASK, CONFIG, GPU, CAMERA_KEY, and leave the rest of the subagent prompt template verbatim.
3. Dispatch a new background subagent — it will pick up prior progress via the checkpoint file at `/tmp/fix_progress_checkpoint_${TASK}_tl.md`
...
| Skill | What to add |
|---|---|
| `.claude/robosuite/training-law/skills/localize.md` | New SAM3/Molmo prompts and strategies that worked for Robosuite objects, API failure fallbacks |
| `.claude/robosuite/training-law/skills/grasp.md` | Grasp strategies, gripper width thresholds, z_offset findings for Robosuite |
| `.claude/robosuite/training-law/skills/transport.md` | Waypoint sequences, bimanual coordination patterns, collision avoidance |
